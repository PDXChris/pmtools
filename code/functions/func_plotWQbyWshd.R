...
#' d <- mergeStatInfo(d)
#' plotWQbyWshd('copper', d)
#' @export


plotWQbyWshd <- function(vbl, dfm=wq14) {
  condition_call <- substitute(metric_name == vbl)
  r <- eval(condition_call, dfm, parent.frame())
  tmp <- dfm[r, ]
  ...
  tmp$watershed <- gsub(' ', '\n', tmp$watershed)
  tmp$watershed <- factor(tmp$watershed, levels=c("Fanno\nCreek", "Tualatin\nStreams",
                                                  "Tryon\nCreek", "Willamette\nStreams", "Johnson\nCreek",
                                                  "Columbia\nSlough"))
  breaks <- as.vector(c(1, 2, 5) %o% 10^(-5:5))

  vlbl <- met.cod$label[(match(vbl, met.cod$metric_name))] #.simpleCap(as.character(lev))
